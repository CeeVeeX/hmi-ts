import { ProtocolError, type IResponse } from '@hmi-ts/core'
 ...
 * ```
 */
export function decodeResponse(frame: Uint8Array): IResponse {
  if (frame.length < 8) {
 ...
 * ```
 */
export function decodeResponseRtu(frame: Uint8Array): IResponse {
  if (frame.length < 5) {
 ...
 * ```
 */
export function decodeResponseAscii(frame: Uint8Array): IResponse {
  if (frame.length < 9) {
 ...
 * ```
 */
export function decodeResponseByMode(frame: Uint8Array, mode: ModbusWireMode): IResponse {
  if (mode === 'rtu') {
